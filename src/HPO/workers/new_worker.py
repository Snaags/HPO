...
def _compute(hyperparameter,cuda_device, JSON_CONFIG ):
  ...
  for _ in range(SETTINGS["REPEAT"]):
    ...
    if SETTINGS["RESAMPLES"]:
      ...
      train_dataset = Train_TEPS(name,device = cuda_device, binary =SETTINGS["BINARY"],augmentations = augs, samples_per_class = 500 )
      test_dataset = Test_TEPS(name,device = cuda_device ,augmentations = False,binary =SETTINGS["BINARY"], samples_per_class = 500)
      splits = [(None,None)]
