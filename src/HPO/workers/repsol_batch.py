...
import pandas as pd
import torch
from HPO.data.repsol_dataset import Repsol_Full
...
def _compute(hyperparameter,budget = 4, in_model = None , train_path = None,  test_path = None, cuda_device = None,plot_queue = None, model_id = None, binary = True):
  ### Configuration 
  THRESHOLD = 0.15 #Cut off for classification

  if cuda_device == None:
     cuda_device = 1# torch.cuda.current_device()

  dataset = Repsol_Full(path_dir ="repsol_mixed/" )
      ...
      if multibatch:
        ...
        for i in batches:
          trainloader[i] = torch.utils.data.DataLoader(
                              ...
                              batch_size=i, drop_last = True,sampler = train_subsampler)
      else:
...
if __name__ == "__main__":
  ...
  hpo = {'batch_size': 2, 'channels': 27, 'jitter': 0.01241258424762939, 'jitter_rate': 0.5439942968995378, 'mix_up': 0.19412584247629389, 'mix_up_rate': 0.5439942968995378, 'cut_mix': 0.19412584247629389, 'cut_mix_rate': 0.5439942968995378, 'cut_out': 0.0941258424762939, 'cut_out_rate': 0.7439942968995378, 'crop': 0.19412584247629389, 'crop_rate': 0.5439942968995378, 'scaling': 0.001317169415702424, 'scaling_rate': 0.4353430973459786, 'window_warp_num': 3, 'window_warp_rate': 1.4001548161604196, 'lr': 0.0025170869707739693, 'p': 0.00296905723528657, 'epochs': 70, 'layers': 3}
