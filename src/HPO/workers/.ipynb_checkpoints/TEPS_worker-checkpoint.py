...
def _compute(hyperparameter,budget = 1, in_model = None , train_path = None,  test_path = None, cuda_device = None,plot_queue = None, model_id = None, binary = True):
  ...
  hpo = {'channels': 128, 'lr': 0.0025170869707739693, 'p': 0.0, 'epochs': 100, 'layers': 3}
  ...
  dataset_train = Train_TEPS(samples_per_class = 500,augmentations = augmentations,device = cuda_device,one_hot = True,binary = binary,samples_per_epoch = 1)
  ...
  for fold in range(budget):
      ...
      ### Train the model
      """
      _ = train_model(model , hpo, trainloader, hpo["epochs"], batch_size , cuda_device, graph = plot_queue, binary = binary,evaluator=evaluator,logger = False) 
      ##with aug
