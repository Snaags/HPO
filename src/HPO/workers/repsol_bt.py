...
import pandas as pd
import torch
from HPO.data.datasets import Test_repsol_full , Train_repsol_full, repsol_unlabeled
...
def _compute(hyperparameter,budget = 4, in_model = None , train_dataset = None, cuda_device = None):
  ...
  if train_dataset == None:
    train_dataset = repsol_unlabeled()
  retrain_dataset = Train_repsol_full(hyperparameter["augmentations"], augmentations = True)
  ...
  retrain_dataloader = DataLoader( retrain_dataset, batch_size=batch_size,
  ...
  ## Train the model
  """
  train_model_bt(model , hyperparameter, train_dataloader , hyperparameter["epochs"], batch_size , cuda_device)
  hyperparameter["lr"] *= 0.05
  train_model(model , hyperparameter, retrain_dataloader , 3, batch_size , cuda_device)
  ...
  print()
  print("Test results -- Correct {}/{} -- Faults Correct {}/{}".format(correct, total, recall_correct ,recall_total)) 
